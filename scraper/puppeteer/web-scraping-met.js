...
(async (
  url = "https://www.metmuseum.org/art/collection/search/590921",
  config = {}
) => {
  ...
  try {
    let checkImage = await page.evaluate(() => {
      ...
      let getCheckLink = document.querySelector(
        ".artwork__image__wrapper .artwork__image"
      )
        ? document
            .querySelector(".artwork__image__wrapper .artwork__image")
            .getAttribute("src")
        : "";
  ...
  let getMedium = await page.evaluate(() => {
    ...
    if (mediumIndex !== -1) {
      // @ts-ignore
      medium = [...document.querySelectorAll(".artwork-tombstone--item *")][
        mediumIndex + 1
      ].textContent;
    }
  ...
  if (getMedium?.length) {
    ...
    mediumTags.map((mediumTag) => {
      if (mediumTag.trim().startsWith("and")) {
        ...
        let splitAnd = [
          ...mediumTag.trim().toLowerCase().replaceAll("  ", " ").split("and"),
        ].map((a) =>
          a
            .trim()
            .replace(new RegExp("[^a-z0-9_ ]", "gi"), " ")
  ...
  let getTitle = await page.evaluate(() => {
    ...
    if (titleIndex !== -1) {
      // @ts-ignore
      title = [...document.querySelectorAll(".artwork-tombstone--item *")][
        titleIndex + 1
      ].textContent;
    }
  ...
  if (getTags?.length) {
    ...
    getTags.map((getTag) => {
      ...
      if (
        !eachTag.includes("all related artworks") &&
        !eachTag.includes("in the same gallery")
      ) {
  ...
  if (description?.length) {
    let keywordDescription = description
      .replace(new RegExp("[^a-z0-9]", "gi"), " ")
      .split(" ");
  ...
  tags.map((tag) =>
    data.tags.push(tag.replace(new RegExp("[^a-z0-9_ ]", "gi"), ""))
  );
