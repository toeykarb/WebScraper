...
const url = 'https://www.loc.gov/item/2017834524/';
console.log('url', url);
const data = {
  // projectTags,
  tags: [],
...
(async (config = {}) => {
  ...
  //   const tags =
  //     (await page.evaluate(
  //       "[...document.querySelectorAll(`.short-list a[data-field-label='Subjects']`)].map(a => a.innerText.trim().toLowerCase().replace(new RegExp('[^a-z0-9]', 'gi'), ''))"
  //     )) || [];
  ...
  // data.tags = data.tags.filter(onlyUnique);
  // console.log(data.tags);
