...
(async (url = "https://open.smk.dk/artwork/image/kksgb10047", config = {}) => {
  ...
  let getArticleId = await page.evaluate(() => {
    ...
    ].findIndex((node) => node.textContent.trim() === "Inventarnummer");
    if (articleIdIndex !== -1) {
      // @ts-ignore
      articleId = [...document.querySelectorAll(".ArtworkItem_artworkItem__2yrsd *")][
        articleIdIndex + 1
      ].textContent;
    }
  ...
  data.isCC0 =
    ...
    licenseText.includes("til fri brug");
  if (data.isCC0) {
    ...
    rawAuthorTag.map((item) => {
      ...
      if (getArtist?.length) {
        data.authorTags.push(
          `#${getArtist.trim().toLowerCase().replace(new RegExp("[^À-ža-z0-9]", "gi"), "").trim()}`
        );
        tags.push(
          `${getArtist
            .trim()
  ...
  let getDateNotes = metadata["production_dates_notes"] ? metadata["production_dates_notes"] : [];
  ...
  let getProductionDate = metadata["production_date"] ? metadata["production_date"] : [];
  ...
  if (getTechniques.length > 0) {
    ...
    let getTechniques = await page.evaluate(() => {
      let techniquesId = "";
      let techniquesIndex = [
        // @ts-ignore
        ...document.querySelectorAll(".ArtworkItem_artworkItem__2yrsd *"),
      ].findIndex((node) => node.textContent.trim() === "Teknik");
      if (techniquesIndex !== -1) {
        // @ts-ignore
        techniquesId = [...document.querySelectorAll(".ArtworkItem_artworkItem__2yrsd *")][
          techniquesIndex + 1
        ].textContent;
      }
    ...
    if (getTechniques?.length) {
      console.log("getTechniques", getTechniques);
      let techniquesTranslate = await googleTranslate(browser, getTechniques);
      console.log("techniquesTranslate", techniquesTranslate);
      var techniquesKeyword = [];
      techniquesKeyword.push(
        ...techniquesTranslate
          .toLowerCase()
  ...
  let getTitle = await page.evaluate(() => {
    ...
    let titleIndex = [
      // @ts-ignore
      ...document.querySelectorAll(".ArtworkItem_artworkItem__2yrsd *"),
    ].findIndex((node) => node.textContent.trim() === "Titel");
    if (titleIndex !== -1) {
      // @ts-ignore
      titleId = [...document.querySelectorAll(".ArtworkItem_artworkItem__2yrsd *")][titleIndex + 1]
        .textContent;
    }
...
const googleTranslate = async (browser, text, timeout = 2000) => {
  ...
  try {
    const allText = [];
    await page2.goto("https://translate.google.com/?sl=da&tl=en&op=translate&text=" + text);
