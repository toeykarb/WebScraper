...
const downloadSkitterphotoFile = require("./test-fetch-burstshopify");
const data = {
  tags: [],
  internalTags: [],
  isCC0: false,
...
(async (config = {}) => {
  ...
  var url =
    "https://skitterphoto.com/photos/10249/narrow-street-restaurants-and-bars-tokyo-japan";
  ...
  const pageCookies = allCookies.cookies.filter((c) =>
    c.domain.includes("skitterphoto")
  );

  // var { getToken, imageLink } = await page.evaluate(() => {
  //   const results = [
  //     ...document.querySelectorAll(".has-sidebar .inline-form"),
  //   ].filter((x) => x.getAttribute("action").toString().endsWith("download"));
